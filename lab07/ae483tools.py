...
def transform_data_mocap(raw_data):
    ...
    for i in range(len(data['time'])):
        ...
        data['yaw'][i], data['pitch'][i], data['roll'][i] = Rotation.from_matrix(R_inW_ofB).as_euler('ZYX')
              # <-- FIXME
...
def sync_data_mocap(raw_data_mocap, t, z_drone):
    ...
    #t_shifts = np.linspace(t[0], t[-1], len(t))
    t_shifts = np.linspace(0., t[-1], 200)
